...
places_raw <- tryCatch(
    ...
    error = function(e) {
        ...
        read_csv("../data/places_county_long.csv", show_col_types = FALSE)
    }
)

if (!dir.exists("../data")) dir.create("../data", recursive = TRUE)
write_csv(places_raw, "../data/places_county_long.csv")
...
places <- places_raw |>
    ...
    ungroup() |>
    select(
        locationid, state_abbr, state_desc, county_name,
        total_population, col_name, data_value
    ) |>
    pivot_wider(
        names_from = col_name, values_from = data_value,
        values_fn = mean
    ) |>
    rename(county_fips = locationid) |>
    filter(!is.na(state_abbr)) |>
    mutate(
        region = region_map[state_abbr],
...
ui <- fluidPage(
    ...
    # ── Top controls bar ────────────────────────────────────────
    div(
        style = "background:#1a3a5c; padding:8px 16px; margin:-8px -15px 10px;
        ...
        div(
            ...
            div(
                style = "color:#ccc; font-size:11px; white-space:nowrap;",
                "Filter by State:"
            ),
            div(
                style = "width:130px;",
    ...
    div(
        ...
        tags$details(
            tags$summary(
                style = "font-size:12px; color:#2980b9; cursor:pointer;",
...
server <- function(input, output, session) {
    ...
    measure_name <- reactive({
        unname(measure_labels[input$main_measure])
    })
    ...
    output$ov_hist <- renderPlotly({
        ...
        plot_ly(
            ...
            layout(
                margin = list(l = 40, r = 8, t = 10, b = 40),
                xaxis = list(
                    title = measure_name(), ticksuffix = "%",
    ...
    output$ov_mini_scatter <- renderPlotly({
        ...
        plot_ly(df,
            x = ~lpa_crudeprev, y = ~ .data[[input$main_measure]],
            ...
            layout(
                margin = list(l = 48, r = 8, t = 10, b = 40),
                xaxis = list(
                    title = "Physical Inactivity (%)", ticksuffix = "%",
                    tickfont = list(size = 10)
                ),
                yaxis = list(
                    title = measure_name(), ticksuffix = "%",
    ...
    output$choropleth <- renderPlotly({
        df <- places |>
            group_by(state_abbr) |>
            summarise(
                avg = mean(.data[[input$main_measure]], na.rm = TRUE),
    ...
    output$lollipop <- renderPlotly({
        ...
        p <- lollipop_df |>
            ggplot(aes(
                ...
                size = highlight,
                text = paste0(
                    county_name, ", ", input$lollipop_state,
                    "\n", m_lb, ": ", round(value, 1), "%",
            ...
            geom_segment(
                aes(
                    xend = reorder(county_name, deviation), y = 0,
                    yend = deviation
                ),
        ...
        ggplotly(p, tooltip = "text") |>
            layout(
                margin = list(l = 5, r = 10, t = 30, b = 40),
